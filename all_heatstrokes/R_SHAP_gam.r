...
RMSE(Obs = Data_train$SeverityAll, Pred = Data_train$Pred)
RMSE(Obs = Data_test$SeverityAll, Pred = Data_test$Pred)
...
Data_train <- Data_train[Pred_sum_date < 300, , ]
Data_train <- data.frame(Data_train)
...
Select_ID_SHAP <- sample(Data_train$ID_SHAP, size = 3000)
...
fwrite(Hoge, "SHAP_all_severity_gam.csv")
